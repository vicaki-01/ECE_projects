...
module PipelinedProcTest(); // Test 2 
...
// should be 64'h70 for the 3rd program test
  ...
  // you should change the testbech to handle the rest 
      if(MemWrite) begin
        if(DataAdr == 64'h58 & WriteData == 64'h1010 )
          $display("Simulation succeeded %d", test);
	else if(DataAdr == 64'h60 & WriteData == 64'd4) 
          $display("Simulation succeeded %d", test);
	else if(DataAdr == 64'h68 & WriteData == 64'hfffffffffffffff6)
          $display("Simulation succeeded %d", test);
	else if(DataAdr == 64'h70 & WriteData == 64'hffffffffffffffe8)
          $display("Simulation succeeded %d", test);
        else
          $display("Simulation failed %d, Actual: %h", test, WriteData);
	test = test + 1;
        end

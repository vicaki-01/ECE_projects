...
// should be 64'h70 for the 3rd program test
  ...
  // you should change the testbech to handle the rest 
      if(MemWrite) begin
        if(DataAdr == 104 & WriteData == 5)
          $display("Simulation succeeded %d", test);
	else if(DataAdr == 112 & WriteData == 5) // Data Hazard, needs to be a nOP
          $display("Simulation succeeded %d", test);
	else if(DataAdr == 120 & WriteData == 7)
          $display("Simulation succeeded %d", test);
	else if(DataAdr == 128 & WriteData == 10)
          $display("Simulation succeeded %d", test);
	else if(DataAdr == 136 & WriteData == 5)
          $display("Simulation succeeded %d", test);
        else
